...
license=('custom')
makedepends=('linux-bede-lts>=4.1.15' 'linux-bede-lts<4.2' 'linux-bede-lts-headers>=4.1' 'linux-bede-lts-headers<4.2')
source=(
    'modprobe.d'
...
install=broadcom-wl-bede-lts.install
...
package() {
    ...
    sed -i -e "s/EXTRAMODULES=.*/EXTRAMODULES='$_extramodules'/" "$startdir/broadcom-wl-bede-lts.install"
}
